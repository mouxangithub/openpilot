...
class CameraFastMJPG:
    def __init__(self, cam_type_state, stream_type, camera_id):
        ...
        # 1. 打开摄像头设备，强制指定输入格式为 mjpeg
        self.container = av.open(
            camera_id,
            format='v4l2',          # Linux 视频设备格式
            options={
                'input_format': 'mjpeg',  # 强制 MJPG 输入格式
                'framerate': '30',        # 设置帧率
                'video_size': '1280x720'  # 设置分辨率
            }
        )
        assert self.container.streams.video, f"Can't open video stream for camera {camera_id}"
    ...
    def read_frames(self):
        ...
        for packet in self.container.demux(self.video_stream):
            for frame in packet.decode():
                # 直接操作 YUV 数据，避免转 RGB/BGR
                if frame.format.name == 'yuvj420p':
                    # 将 YUVJ420P（MJPG 解码后的常见格式）转为标准 YUV420P
                    frame = frame.reformat(format='yuv420p')
                # 转换为 NV12 格式
                nv12_frame = frame.reformat(format='nv12')
                yield nv12_frame.to_ndarray().tobytes()
