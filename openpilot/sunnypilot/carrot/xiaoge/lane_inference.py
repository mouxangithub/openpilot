...
Ported from CarrotPilot (cp):
  selfdrive/carrot/xiaoge/lane_inference.py

Requirements: opencv-python-headless >= 4.10.0, numpy >= 1.24
"""
...
class LaneCandidate:
  def __init__(self, class_id: int, score: float, bottom: int,
               center_at_bottom: float, mask: np.ndarray | None = None):
...
def non_maximum_suppression(
  boxes_left: np.ndarray, boxes_top: np.ndarray,
  boxes_right: np.ndarray, boxes_bottom: np.ndarray,
  scores: np.ndarray, class_ids: np.ndarray, iou_threshold: float,
) -> np.ndarray:
  ...
  for class_id in np.unique(class_ids):
    ...
    while len(order):
      ...
      if not len(remaining):
        break
      intersection_w = np.maximum(0.0, np.minimum(boxes_right[current], boxes_right[remaining]) -
                                  np.maximum(boxes_left[current], boxes_left[remaining]))
      intersection_h = np.maximum(0.0, np.minimum(boxes_bottom[current], boxes_bottom[remaining]) -
  ...
  return np.array(sorted(kept_indices, key=lambda idx: scores[idx], reverse=True), dtype=np.intp)
...
class LaneInference:
  ...
  def load(self) -> bool:
    if cv2 is None:
      self.error = "OpenCV (cv2) is missing - install opencv-python-headless"
      self.valid = False
      return False
  ...
  def infer(self, image: np.ndarray, width: int, height: int,
            conf_thresh: float = CONFIDENCE_THRESHOLD,
            iou_thresh: float = IOU_THRESHOLD) -> dict:
    if not self.valid or self.net is None:
      return {
        "leftLine": -1, "rightLine": -1, "leftConf": 0.0, "rightConf": 0.0,
        "valid": False, "error": self.error or "Model not loaded", "candidatesCount": 0,
      }
    try:
      ...
      if not np.any(mask_valid):
        return {
          "leftLine": -1, "rightLine": -1, "leftConf": 0.0, "rightConf": 0.0,
          "valid": True, "error": "", "candidatesCount": 0,
        }
      ...
      coeffs = predictions[10:42, valid_indices].T
      ...
      for k_idx in kept_indices:
        ...
        proto_crop = prototypes[:, top_p:bottom_p + 1, left_p:right_p + 1]
        mask_crop = np.tensordot(coeff, proto_crop, axes=(0, 0)) > 0.0
      ...
      return {
        "leftLine": left_type, "rightLine": right_type,
        "leftConf": round(left_conf, 3), "rightConf": round(right_conf, 3),
        "valid": True, "error": "", "candidatesCount": len(candidates),
      }

    except Exception as exc:
      return {
        "leftLine": -1, "rightLine": -1, "leftConf": 0.0, "rightConf": 0.0,
        "valid": False, "error": str(exc), "candidatesCount": 0,
      }
