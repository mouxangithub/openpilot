...
Ported from CarrotPilot (cp):
  selfdrive/carrot/xiaoge_data.py

Subsystem:
  - Starts xiaoge/v_asm_server.py in a daemon thread. This service reads
    VisionIPC camera buffers (wide road + road cameras), runs ONNX inference
    (blind-spot detection on wide camera, lane classification on road camera),
    and publishes results via customReservedRawData0.
  - card.py consumes customReservedRawData0 and merges results into
    carState / carStateSP.

On-device: requires VisionIPC (real camera hardware).
On PC: VisionIPC unavailable, camera threads will wait and report errors.
"""
import json
import socket
import struct
...
class XiaogeDataBroadcaster:
  ...
  def start_vision_server(self) -> None:
    from openpilot.sunnypilot.carrot.xiaoge.v_asm_server import create_server
