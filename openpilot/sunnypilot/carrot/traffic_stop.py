...
"""
Traffic-stop model-lead matcher.

Ported and adapted from CarrotPilot's selfdrive/carrot/traffic_stop.py. The
module is intentionally pure-Python (standard library + numpy) so it can be
unit-tested without a compiled capnp/cereal runtime.

It recognizes when the E2E driving model places its ego stop endpoint about
``MODEL_LEAD_STOP_OFFSET_M`` behind a stationary model-detected lead and, once
that relationship is stable over ``MODEL_LEAD_STOP_CONFIRM_FRAMES`` frames,
exposes the vehicle's approximate position as the MPC obstacle. It never
publishes or promotes a radar lead; it is purely an E2E obstacle correction.
"""
