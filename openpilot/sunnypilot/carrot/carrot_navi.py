...
try:
  ...
  import types
  def _app_key(name: str, type_: Any) -> Any:
    return name
  ...
  web = _AIOHTTP_STUB
  WSMsgType = _AIOHTTP_STUB.MsgType  # type: ignore[misc]
...
class ClusterNaviMapParamReader:
  ...
  sunnypilot reuses the upstream CarrotPilot param names where available
  (ClusterNaviMapTheme / ClusterNaviMapType / ClusterNaviMapFps /
  CarrotNaviHudMapProfile). Any key that is not yet registered falls back to a
  safe default, so the 7714 link works even before those keys are provisioned.
  """
...
def main() -> None:
  ...
  if not args.no_cereal:
    try:
      from openpilot.sunnypilot.carrot.carrot_navi_cereal import CarrotNaviCerealPublisher
