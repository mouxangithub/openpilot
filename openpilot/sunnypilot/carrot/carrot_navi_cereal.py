...
if TYPE_CHECKING:
  from openpilot.sunnypilot.carrot.carrot_navi import CarrotNaviReceiver


# sunnypilot publishes the 7714 v2 link under SP-namespaced cereal services so
# it cannot collide with the upstream `carrotNavi` / `carrotNaviMedia` names.
SCHEMA_VERSION = 1
...
SERVICE_NAVI = "carrotNaviSP"
SERVICE_MEDIA = "carrotNaviMediaSP"
...
class CarrotNaviCerealPublisher:
  def __init__(self, receiver: CarrotNaviReceiver, messaging_module: Any | None = None,
    ...
    self.pm = messaging_module.PubMaster([SERVICE_NAVI, SERVICE_MEDIA])
    if params is None:
      try:
  ...
  def publish_once(self, snapshot: dict[str, Any] | None = None) -> int:
    ...
    message = self.messaging.new_message(SERVICE_NAVI, valid=True)
    message.carrotNaviSP = build_carrot_navi_payload(snapshot)
    self.pm.send(SERVICE_NAVI, message)
  ...
  def publish_media(self, record: Any, session_id: str, kind_override: str | None = None) -> None:
    message = self.messaging.new_message(SERVICE_MEDIA, valid=True)
    payload = build_carrot_navi_media_payload(record, session_id)
    if kind_override is not None:
      payload["kind"] = kind_override
    message.carrotNaviMediaSP = payload
    self.pm.send(SERVICE_MEDIA, message)
    self.receiver.record_cereal_publish()
