...
Ported from CarrotPilot (cp). See bluetooth/__init__.py for architecture overview.
"""
...
COMMAND_TTL = 0.4
BLUETOOTH_CANCEL = -3  # Explicit driver cancel; -1/-2 retain their existing meanings.
DEFAULT_MAPPING = {
  'up': 'accelCruise', 'down': 'decelCruise', 'left': 'laneLeft', 'right': 'laneRight',
  'center': 'paddleDecel', '1': 'gapAdjustCruise', '2': 'none'
}
MAC_RE = re.compile(r'^(?:[0-9A-F]{2}:){5}[0-9A-F]{2}$')
TOKEN_RE = re.compile(
  r'^(?:up|down|left|right|center|1|2|key:[0-9]{1,4}|swipe:[xy][+-]|tap:[0-9]{1,5}:[0-9]{1,5})'
  r'(?:@double|@long)?$'
)
...
class AtomicJSON:
  """Alias for the atomic_json module-level function, for public API compatibility."""
  __slots__ = ()
  def __call__(self, path, value, mode=0o600):
    return atomic_json(path, value, mode)
...
def validate_config(data):
  ...
  for mac, device in data.get('devices', {}).items():
    ...
    for key, action in mapping.items():
      if not TOKEN_RE.fullmatch(key) or action not in ACTIONS:
    ...
    result['devices'][mac] = {
      'name': str(device.get('name', mac))[:80],
...
class Clicks:
  ...
  __slots__ = ('mapping', 'learning', 'pending')
...
class Decoder:
  ...
  __slots__ = (
    'profile', 'x', 'y', 'touch', 'start', 'last', 'started', 'down',
    'pending_keys', 'dropped', 'clicks', 'holds', 'frame_open', 'repeated',
  )
  ...
  def hold(self, source, token, started):
    ...
    if not previous and token:
      self.holds[source] = {
        'token': token, 'started': started, 'last': None, 'fired': False, 'expired': False
      }
  ...
  def touch_token(self):
    ...
    if max(abs(dx), abs(dy)) >= 120:
      ...
      return {'swipe:y+': 'up', 'swipe:y-': 'down',
              'swipe:x+': 'left', 'swipe:x-': 'right'}.get(token, token) if self.profile == 'yiser-j6' else token
...
class CommandWriter:
  ...
  __slots__ = ('root', 'events', 'session', 'sequence')
  ...
  def prune(self, addresses, now, active_holds=None):
    for channel, events in self.events.items():
      kept = [
        e for e in events
        if e['address'] in addresses
        and 0 <= now - e['time'] <= COMMAND_TTL
        and (not e.get('hold') or active_holds is None or e['hold'] in active_holds)
      ]
      if kept != events:
  ...
  def send(self, mac, action, now, hold=None, repeat=False):
    ...
    events = [
      ...
      if 0 <= now - e['time'] <= COMMAND_TTL and (not hold or e.get('hold') != hold)
    ]
    events.append({
      'id': f'{self.session}:{self.sequence}',
      'time': now,
      'action': action,
      'address': mac,
      'hold': hold,
...
class CommandReader:
  ...
  __slots__ = ('path', 'started', 'last_id', 'seen', 'last_check', 'is_repeat')
  ...
  def read(self, allowed=True, now=None):
    ...
    for message in messages[-64:]:
      ...
      if not isinstance(created, (int, float)) \
         or not self.started <= created <= now \
         or now - created > COMMAND_TTL:
        continue
