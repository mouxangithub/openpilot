...
Ported from CarrotPilot (cp). Requires jeepney for D-Bus async support.

D-Bus bus: SYSTEM (not session). Requires write access to /data/bluetooth/ENABLED
and bluetooth service control (systemctl start/stop bluetooth). Typically accessed
via sudo in the HTTP API layer.
"""
import asyncio
...
from openpilot.sunnypilot.carrot.bluetooth.model import AddressValidator
...
class Bluez:
  """Async BlueZ D-Bus client for Bluetooth device management."""

  __slots__ = (
    'context', 'router', 'agent_task', 'pair_task', 'scan_task',
    'lock', 'agent_registered', 'owner', 'target', 'prompt', 'answer', 'pair',
  )
  ...
  async def call(self, path, interface, method, signature=None, body=(), reply_timeout=10):
    msg = new_method_call(
      DBusAddress(path, bus_name='org.bluez', interface=interface),
      method, signature, body,
    )
    return unwrap_msg(await asyncio.wait_for(self.router.send_and_get_reply(msg), reply_timeout))
  ...
  async def locate(self, mac):
    mac = AddressValidator.validate(mac)
    for path, interfaces in (await self.objects()).items():
      if DEVICE in interfaces and _props(interfaces[DEVICE]).get('Address', '').upper() == mac:
        return path
  ...
  async def snapshot(self):
    ...
    for _path, interfaces in (await self.objects()).items():
      ...
      if DEVICE in interfaces:
        ...
        # Battery level (org.bluez.Battery1, optional)
        if 'org.bluez.Battery1' in interfaces:
          devs['battery'] = _props(interfaces['org.bluez.Battery1']).get('Percentage')
        devices.append(devs)
  ...
  async def device_action(self, mac, action):
    ...
    if action == 'forget':
      ...
      await self.call(
        path, DEVICE,
        {'connect': 'Connect', 'disconnect': 'Disconnect'}[action],
        reply_timeout=30,
      )
  ...
  async def start_pair(self, mac):
    ...
    self.target = path
    self.pair = {'state': 'pairing', 'address': AddressValidator.validate(mac)}

    async def run():
      try:
        await self.call(path, DEVICE, 'Pair', reply_timeout=90)
        await self.call(path, 'org.freedesktop.DBus.Properties', 'Set',
                         'ssv', (DEVICE, 'Trusted', ('b', True)))
        self.pair = {'state': 'paired', 'address': AddressValidator.validate(mac)}
        try:
          await self.call(path, DEVICE, 'Connect', reply_timeout=20)
        except Exception as exc:
          self.pair['error'] = str(exc)
      except asyncio.CancelledError:
        self.pair = {'state': 'cancelled', 'address': AddressValidator.validate(mac)}
        raise
      except Exception as exc:
        self.pair = {'state': 'error', 'address': AddressValidator.validate(mac), 'error': str(exc)}
      finally:
  ...
  def respond(self, prompt_id, value):
    if not self.prompt or self.prompt['id'] != prompt_id \
       or not self.answer or self.answer.done():
  ...
  async def agent_reply(self, message):
    ...
    try:
      if message.header.fields.get(HeaderFields.sender) != self.owner \
         or message.header.fields.get(HeaderFields.interface) != 'org.bluez.Agent1':
        ...
        self.prompt = {
          'id': uuid.uuid4().hex,
          'kind': method,
          'value': str(message.body[1]).zfill(6) if method == 'DisplayPasskey' else str(message.body[1]),
        }
        reply = new_method_return(message)
