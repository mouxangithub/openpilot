...
class _MockParams:
  """In-memory params stand-in for ClusterNaviMapParamReader tests."""
  ...
  def get_param_path(self, key: str) -> str:
    # Intentionally return a non-existent path so the fallback default is used.
    return f"/nonexistent/param/path/{key}"


class TestClusterNaviMapParamReader(unittest.TestCase):
  def test_defaults_match_registered_params(self):
    reader = carrot_navi.ClusterNaviMapParamReader(_MockParams({}))
    theme, map_type, map_hz, bitrate, center_y = reader.read()
    self.assertEqual(theme, "dark")
    self.assertEqual(map_type, "normal")
    self.assertEqual(map_hz, 10)
    self.assertEqual(bitrate, 3000)
    self.assertEqual(center_y, carrot_navi.MAP_SCREEN_CENTER_Y_RATIO_DEFAULT)

  def test_hud_profile_uses_hud_center_ratio(self):
    reader = carrot_navi.ClusterNaviMapParamReader(_MockParams({
    ...
    self.assertEqual(map_type, "satellite")
    self.assertEqual(map_hz, 30)
    self.assertEqual(center_y, carrot_navi.MAP_SCREEN_CENTER_Y_RATIO_HUD)

  def test_unknown_option_values_fall_back(self):
    reader = carrot_navi.ClusterNaviMapParamReader(_MockParams({
      "ClusterNaviMapTheme": 99,
      "ClusterNaviMapType": 99,
      "ClusterNaviMapFps": 99,
    }))
    theme, map_type, map_hz, _, _ = reader.read()
    self.assertEqual(theme, "dark")
    self.assertEqual(map_type, "normal")
    self.assertEqual(map_hz, carrot_navi.MAP_HZ_DEFAULT)
