...
kept whole rather than reduced to a single argmax curvature lookup: the three
things that make it work are all load-bearing, and dropping any one of them
changes the behaviour in a way that is easy to miss on a straight road and
obvious in a tightening corner.

What it does that a bare ``v = sqrt(a_lat / kappa)`` cannot:

  * A three-node median rejects isolated yaw spikes before any minimum is taken,
    so one noisy model frame cannot anchor the whole approach.
  * The speed is a *distance-adjusted envelope*: for each candidate point it
    computes the speed that would still allow a comfortable stop-free approach
    from here, so the car starts slowing before the corner rather than at it.
  * ``VisionCurveSpeed`` holds the ceiling and releases it only against a short
    history of fresh geometry, so a single new frame cannot un-limit the car.

Model velocity is used only to recover curvature from yaw rate. It is never
treated as a desired vehicle speed. All calculations use metres and seconds; the
result is converted to the cruise/display convention at the boundary, i.e. km/h,
which is what ``CarrotPlanner.carrot_curve_speed`` has always returned.
"""
...
def curve_speed(model, v_ego, sensitivity=1.0, lower_limit_kph=30.0, *, speed_ratio=1.0, a_ego=0.0):
  ...
  Returns a ``CurveSpeed``, or ``None`` when the path is unusable - which is not
  the same as "no limit": the caller distinguishes them, and a missing result
  must not be read as a straight road.
  """
...
class VisionCurveSpeed:
  ...
  The asymmetry is the point: slowing for a curve the moment the model sees it is
  always safe, while releasing is a claim about the road ahead that a single
  frame does not support.
  """
  def __init__(self):
