#!/usr/bin/env python3
"""Control-facing gating/normalisation for the 7714 WebSocket v2 navi link.

This module turns a parsed ``CarrotNaviStateSP`` payload (a plain dict, as
produced by :mod:`openpilot.sunnypilot.carrot.carrot_navi_cereal`) into a small
set of immutable, control-friendly dataclasses. It performs the safety gating
described in the migration plan:

  * ``schemaVersion != 1`` or ``connected == False`` -> ``None``
  * off-route suppresses guidance distance fields and SDI distances
  * a section only contributes when active, not suspended/off-route, limit > 0
  * road limits are filtered to sane 10-multiple kph values
  * traffic lamp priority is red > left > green > right > uturn

The 7714 v2 path must never apply a speed decision before the fresh
``road_category`` has been captured (the upstream type-22 path had this bug);
here ``road_category`` is read from ``laneCurrent`` and returned alongside the
speed block, with no SDI application happening inside this gating layer.
"""
...
def parse_carrot_navi_control(data: Any) -> CarrotNaviControl | None:
  # Gate 1: protocol/connection validity.
  ...
  # Capture the fresh road_category from laneCurrent up-front, so any future
  # consumer can evaluate SDI against the current road class without the
  # type-22 ordering bug (speed applied before road_category was read).
