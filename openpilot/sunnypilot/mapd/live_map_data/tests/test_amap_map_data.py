...
class TestAmapCurveSpeed(unittest.TestCase):
  ...
  def test_curve_speed_is_opt_in(self):
    ...
    self.assertEqual(off.get_curve_speed_and_distance(), (0.0, 0.0))

    on = _provider(AmapCurveSpeedEnabled=True)
    with patch("openpilot.sunnypilot.mapd.live_map_data.amap_map_data._http_get_json", return_value=payload):
      on._update_speed_limits("KEY", "116.400000,39.900000")
    # A straight road is reported as CURVE_MAX_SPEED_KPH by the geometry chain, which
    # means "no curve constraint" - not "a 130 kph limit". The provider normalises that
    # to 0 so a consumer cannot mistake it for a real cap, and the valid flag (see
    # get_curve_speed_and_distance) is what carries the distinction.
    self.assertEqual(on._curve_speed_limit, 0.0)

  def test_curve_speed_is_in_metres_per_second(self):
    """The stored value must be m/s, matching _speed_limit and the SCC consumer.

    Regression guard: this field previously held the raw kph figure from the geometry
    chain, 3.6x too high, while its comment and its getter both claimed m/s. Nothing
    read it, so the error was invisible - the kind of latent fault that only surfaces
    once a consumer is wired up (which is what this change does).

    The expected value is derived from the *parsed* polyline, not from the raw
    coordinates: the payload rounds to 6 decimal places, so fitting the circle to the
    pre-rounding points would compare against a number the provider never sees.
    """
    import math
    tight = []
    for deg in range(0, 90, 3):
      ...
      tight.append((39.9 + 30.0 * math.cos(rad) / 111132.92,
                    116.4 + 30.0 * math.sin(rad) / 85300.0))
    poly = ";".join(f"{lng:.6f},{lat:.6f}" for lat, lng in tight)
    payload = {"status": "1", "route": {"paths": [{"steps": [{"distance": "500", "polyline": poly}]}]}}

    p = _provider(AmapCurveSpeedEnabled=True)
    with patch("openpilot.sunnypilot.mapd.live_map_data.amap_map_data._http_get_json", return_value=payload):
      p._update_speed_limits("KEY", "116.400000,39.900000")

    expected_kph = _min_curve_speed_kph(_parse_polyline(poly), CURVE_LOOKAHEAD_M)
    self.assertLess(expected_kph, CURVE_MAX_SPEED_KPH, "fixture must be a real curve")
    self.assertGreater(expected_kph, CURVE_MIN_SPEED_KPH - 1e-6)
    self.assertAlmostEqual(p._curve_speed_limit, _kph_to_ms(expected_kph), places=6)
    # And specifically: not the raw kph value.
    self.assertNotAlmostEqual(p._curve_speed_limit, expected_kph, places=3)

  def test_curve_speed_and_distance_reports_the_window(self):
    import math
    tight = []
    for deg in range(0, 90, 3):
      rad = math.radians(deg)
    ...
    with patch("openpilot.sunnypilot.mapd.live_map_data.amap_map_data._http_get_json", return_value=payload):
      p._update_speed_limits("KEY", "116.400000,39.900000")
    speed, dist = p.get_curve_speed_and_distance()
    self.assertGreater(speed, 0.0)
    # The distance is the lookahead window, not a per-curve point distance: the
    # geometry chain reports the lowest safe speed anywhere inside the window.
    self.assertEqual(dist, CURVE_LOOKAHEAD_M)

  def test_straight_road_publishes_no_curve_constraint(self):
    """No constraint must be (0.0, 0.0), so a consumer cannot read it as a cap."""
    poly = ";".join(f"{lng:.6f},{lat:.6f}" for lat, lng in
                    [(39.9000, 116.4000), (39.9015, 116.4000), (39.9030, 116.4000), (39.9045, 116.4000)])
    payload = {"status": "1", "route": {"paths": [{"steps": [{"distance": "500", "polyline": poly}]}]}}
    p = _provider(AmapCurveSpeedEnabled=True)
    with patch("openpilot.sunnypilot.mapd.live_map_data.amap_map_data._http_get_json", return_value=payload):
      p._update_speed_limits("KEY", "116.400000,39.900000")
    self.assertEqual(p.get_curve_speed_and_distance(), (0.0, 0.0))
