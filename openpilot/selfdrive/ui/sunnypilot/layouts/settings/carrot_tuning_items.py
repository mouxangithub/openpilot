...
def build_display_items():
  return [
    ...
    toggle_item_sp(title=tr('Cluster HUD'), param='ClusterHud', description=tr('Requires a TURZX USB cluster display, which is not supported in this build yet. These settings are stored but have no effect until the display is added.')),
    ...
    option_item_sp(title=tr('Cluster HUD Radar Source Color'), param='ClusterHudRadarSourceColor', min_value=0, max_value=1, value_change_step=1),
