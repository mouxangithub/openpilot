...
def compute_yaw_correction(
  ...
  if len(pred_angles) < min_camera_frames:
...
class ImuCalibrator:
  ...
  def get_incremental_rotation(self) -> np.ndarray | None:
    ...
    if self.R_static is None or self.gyro_bias is None:
      return None
    if len(self.dynamic_cam_rot) < DYNAMIC_MIN_INCREMENTAL_CAMERA_FRAMES:
      return None
  ...
  def _to_device_frame(v: list[float] | np.ndarray) -> np.ndarray:
    ...
    Both locationd.py and the C++ locationd_llk remap sensor readings with
    [-v[2], -v[1], -v[0]] before feeding them to the Kalman filter. The
    calibration matrix must be expressed in that same device frame.
    """
  ...
  def handle_accel(self, msg: log.SensorEventData) -> None:
    ...
    acc = self._to_device_frame(msg.acceleration.v)
    self.static_buffer.append(acc, np.zeros(3), ts)

  def handle_gyro(self, msg: log.SensorEventData) -> None:
    ts = msg.timestamp * 1e-9
    gyro = self._to_device_frame(msg.gyroUncalibrated.v)
    if self.state == CalibrationState.STATIC_COLLECTING:
      self.static_buffer.append(np.zeros(3), gyro, ts)
    elif self.state == CalibrationState.DYNAMIC_COLLECTING:
      self.dynamic_gyro.append(np.zeros(3), gyro, ts)
...
class ImuCalibrationD:
  ...
  def _publish(self) -> None:
    ...
    # During dynamic collection, estimate yaw incrementally from the straight-
    # driving data collected so far and publish the full rotation matrix as soon
    # as enough data is available. Avoid publishing R_static with yaw fixed to 0.
    if R is None and self.calibrator.state == CalibrationState.DYNAMIC_COLLECTING:
      R = self.calibrator.get_incremental_rotation()
